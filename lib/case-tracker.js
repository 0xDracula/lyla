...
export async function getCaseActionReportKeysByUser(targetUserId) {
  const rows = await db
    .select({ data: caseActions.data })
    .from(caseActions)
    .where(eq(caseActions.targetUserId, targetUserId));
  return new Set(
    rows
      .map((r) => (r.data?.permalink ? `${r.data.permalink}::${r.data?.whatTheyDid ?? ""}` : null))
      .filter(Boolean)
  );
}
...
export async function updateCaseAction(id, actorId, changes) {
  ...
  if (updated) syncActionToUcat(updated);

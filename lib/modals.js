...
import { timeAgo, truncateToWordBoundary, escapeMrkdwn } from "./slack-utils.js";
...
export function buildUserNotesView({ targetUserId, notes }) {
  const noteBlocks = notes.flatMap((note) => {
    const authorLine = `by <@${note.createdBy}>, ${timeAgo(note.createdAt)}${note.editedBy ? ` (edited by <@${note.editedBy}>, ${timeAgo(note.editedAt)})` : ""
      }`;
    return [
      {
        type: "section",
...
export function buildUserInfoView({
  ...
  const actionBlocks =
    actions.length === 0
      ...
      : actions.map((a) => {
        const label = ACTION_TYPE_LABELS[a.actionType] ?? a.actionType;
        const categoryName =
          a.categoryCode != null ? categoryNameByCode.get(a.categoryCode) : null;
        const categorySuffix = categoryName ? ` - _${escapeMrkdwn(categoryName)}_` : "";
        const by = a.performedBy.map((id) => `<@${id}>`).join(", ");
        const when = timeAgo(a.performedAt);
        return {
          type: "section",
          text: {
            type: "mrkdwn",
            text: `*${label}* on case #\u200c${a.caseNumber} by ${by} (${when})${categorySuffix}`,
          },
        };
      });
  ...
  const AIRTABLE_DISPLAY_CAP = 10;
  const airtableBlocks = airtableRecords.slice(0, AIRTABLE_DISPLAY_CAP).flatMap((record) => {
    const date = record.date
      ...
      : "unknown date";
    const dealtWithBy = record.dealtWithBy ? escapeMrkdwn(record.dealtWithBy) : "unknown";
    const lines = [
      `*Report from ${date}*`,
      `*Dealt With By:* ${dealtWithBy}`,
      `*What Did User Do:* ${escapeMrkdwn(record.whatDidUserDo ?? "")}`,
      `*How Was This Resolved:* ${escapeMrkdwn(record.howResolved ?? "")}`,
    ];
    if (record.permalink) lines.push(`<${record.permalink}|View Message>`);
    return [{ type: "section", text: { type: "mrkdwn", text: lines.join("\n") } }];
  });
  if (airtableRecords.length > AIRTABLE_DISPLAY_CAP) {
    airtableBlocks.push({
      type: "context",
      elements: [
        {
          type: "mrkdwn",
          text: `_...and ${airtableRecords.length - AIRTABLE_DISPLAY_CAP} more — check Airtable directly_`,
        },
      ],
    });
  }
  ...
  return {
    ...
    blocks: [
      ...
      ...auditBlocks,
      ...(airtableBlocks.length > 0
        ? [
          { type: "divider" },
          {
            type: "section",
            text: { type: "mrkdwn", text: "*Legacy Airtable Reports* (pre-migration)" },
          },
          ...airtableBlocks,
        ]
        : []),
...
export async function assembleUserInfoView(
  ...
  const [actionsPage, notesPage, auditPage, categories] = await Promise.all([
  ...
  // Airtable is only worth checking when Postgres has nothing at all for this
  // user — anyone with any Postgres history has been fully migrated already.
  const hasPostgresHistory = actionsPage.actions.length > 0 || notesPage.notes.length > 0;
  const airtableRecords = hasPostgresHistory ? [] : await getLegacyAirtableRecords(targetUserId);

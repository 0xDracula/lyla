...
function collectPayload(payload, tableId, fieldNamesById, pendingUpdates, pendingDeletes) {
  ...
  for (const [recordId, change] of Object.entries(tableChanges.changedRecordsById ?? {})) {
    collectChange(
      ...
      pendingDeletes,
      recordId,
      change.current?.cellValuesByFieldId,
      fieldNamesById,
      actorId
    );
  }
  ...
  for (const recordId of tableChanges.destroyedRecordIds ?? []) {
    pendingDeletes.set(recordId, actorId);
    pendingUpdates.delete(recordId);
  }
}

async function applyPending(pendingUpdates, pendingDeletes) {
  for (const [recordId, { fields, actorId }] of pendingUpdates) {
    try {
  ...
  for (const [recordId, actorId] of pendingDeletes) {
    try {
      const existing = await getCaseActionByAirtableRecordId(recordId);
      if (!existing) continue;
      await deleteCaseAction(existing.id, actorId);
    } catch (error) {
      console.error(`[airtable-webhook] failed to delete for ${recordId}:`, error.message);
    }
  }
}
...
async function fetchAndApplyPayloads() {
  ...
  while (mightHaveMore) {
    ...
    for (const payload of payloads) {
      collectPayload(payload, tableId, fieldNamesById, pendingUpdates, pendingDeletes);
    }
  ...
  await applyPending(pendingUpdates, pendingDeletes);
